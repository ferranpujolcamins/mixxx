...
QString toQStringFirstNotEmpty(const TagLib::ID3v2::FrameList& frameList) {
    for (const TagLib::ID3v2::Frame* pFrame : frameList) {
        if (pFrame) {
            ...
            if (pUnknownFrame) {
                kLogger.warning() << "Unsupported ID3v2 frame" << pUnknownFrame->frameID().data();
            }
        }
    }
    return QString();
...
inline QString formatReplayGainGain(const ReplayGain& replayGain) {
...
bool parseReplayGainGain(ReplayGain* pReplayGain, const QString& dbGain) {
...
bool parseTrackGain(TrackMetadata* pTrackMetadata, const QString& dbGain) {
    DEBUG_ASSERT(pTrackMetadata);
...
inline QString formatReplayGainPeak(const ReplayGain& replayGain) {
    return ReplayGain::peakToString(replayGain.getPeak());
}

inline bool hasTrackPeak(const TrackMetadata& trackMetadata) {
...
bool parseReplayGainPeak(ReplayGain* pReplayGain, const QString& strPeak) {
    DEBUG_ASSERT(pReplayGain);
...
bool parseTrackPeak(TrackMetadata* pTrackMetadata, const QString& strPeak) {
    DEBUG_ASSERT(pTrackMetadata);
...
inline bool hasAlbumGain(const TrackMetadata& trackMetadata) {
    return trackMetadata.getAlbumInfo().getReplayGain().hasRatio();
}

inline QString formatAlbumGain(const TrackMetadata& trackMetadata) {
    return formatReplayGainGain(trackMetadata.getAlbumInfo().getReplayGain());
}

bool parseAlbumGain(TrackMetadata* pTrackMetadata, const QString& dbGain) {
    DEBUG_ASSERT(pTrackMetadata);
...
inline bool hasAlbumPeak(const TrackMetadata& trackMetadata) {
    return trackMetadata.getAlbumInfo().getReplayGain().hasPeak();
}

inline QString formatAlbumPeak(const TrackMetadata& trackMetadata) {
...
void readAudioProperties(
        TrackMetadata* pTrackMetadata, const TagLib::AudioProperties& audioProperties) {
    DEBUG_ASSERT(pTrackMetadata);
...
inline QImage loadImageFromByteVector(
        const TagLib::ByteVector& imageData, const char* format = nullptr) {
    return QImage::fromData(
            // char -> uchar
            reinterpret_cast<const uchar*>(imageData.data()), imageData.size(), format);
}

inline QImage loadImageFromID3v2PictureFrame(const TagLib::ID3v2::AttachedPictureFrame& apicFrame) {
    return loadImageFromByteVector(apicFrame.picture());
}

inline QImage loadImageFromVorbisCommentPicture(const TagLib::FLAC::Picture& picture) {
    return loadImageFromByteVector(picture.data(), picture.mimeType().toCString());
}

bool parseBase64EncodedVorbisCommentPicture(
        TagLib::FLAC::Picture* pPicture, const TagLib::String& base64Encoded) {
...
inline QImage parseBase64EncodedVorbisCommentImage(const TagLib::String& base64Encoded) {
    const QByteArray decodedData(QByteArray::fromBase64(base64Encoded.toCString()));
    return QImage::fromData(decodedData);
}

TagLib::String::Type getID3v2StringType(
        const TagLib::ID3v2::Tag& tag, bool isNumericOrURL = false) {
...
TagLib::ID3v2::CommentsFrame* findFirstCommentsFrame(
        const TagLib::ID3v2::Tag& tag, const QString& description, bool preferNotEmpty = true) {
    TagLib::ID3v2::CommentsFrame* pFirstFrame = nullptr;
    // Bind the const-ref result to avoid a local copy
    const TagLib::ID3v2::FrameList& commentsFrames = tag.frameListMap()["COMM"];
...
TagLib::ID3v2::UserTextIdentificationFrame* findFirstUserTextIdentificationFrame(
    ...
    // Bind the const-ref result to avoid a local copy
    const TagLib::ID3v2::FrameList& textFrames = tag.frameListMap()["TXXX"];
    for (TagLib::ID3v2::FrameList::ConstIterator it = textFrames.begin(); it != textFrames.end();
         ++it) {
...
TagLib::ID3v2::UniqueFileIdentifierFrame* findFirstUniqueFileIdentifierFrame(
        const TagLib::ID3v2::Tag& tag, const QString& owner, bool preferNotEmpty = true) {
    DEBUG_ASSERT(!owner.isEmpty());
    TagLib::ID3v2::UniqueFileIdentifierFrame* pFirstFrame = nullptr;
    // Bind the const-ref result to avoid a local copy
    const TagLib::ID3v2::FrameList& ufidFrames = tag.frameListMap()["UFID"];
    for (TagLib::ID3v2::FrameList::ConstIterator it = ufidFrames.begin(); it != ufidFrames.end();
         ++it) {
        auto pFrame = dynamic_cast<TagLib::ID3v2::UniqueFileIdentifierFrame*>(*it);
        if (pFrame) {
...
inline QString readFirstUserTextIdentificationFrame(
        const TagLib::ID3v2::Tag& tag, const QString& description) {
...
inline QByteArray readFirstUniqueFileIdentifierFrame(
        const TagLib::ID3v2::Tag& tag, const QString& owner) {
...
int removeUserTextIdentificationFrames(TagLib::ID3v2::Tag* pTag, const QString& description) {
    DEBUG_ASSERT(pTag);
    DEBUG_ASSERT(!description.isEmpty());
    int count = 0;
    bool repeat;
    do {
        repeat = false;
        // Bind the const-ref result to avoid a local copy
        const TagLib::ID3v2::FrameList& textFrames = pTag->frameListMap()["TXXX"];
        for (TagLib::ID3v2::FrameList::ConstIterator it(textFrames.begin()); it != textFrames.end();
             ++it) {
            auto pFrame = dynamic_cast<TagLib::ID3v2::UserTextIdentificationFrame*>(*it);
            if (pFrame) {
                ...
                if (0 == frameDescription.compare(description, Qt::CaseInsensitive)) {
                    if (kLogger.debugEnabled()) {
                        kLogger.debug()
                                << "Removing ID3v2 TXXX frame:" << toQString(pFrame->description());
                    }
...
void writeID3v2TextIdentificationFrame(
        TagLib::ID3v2::Tag* pTag, const TagLib::ByteVector& id, const QString& text,
    ...
    if (!text.isEmpty()) {
        ...
        const TagLib::String::Type stringType = getID3v2StringType(*pTag, isNumericOrURL);
        auto pFrame = std::make_unique<TagLib::ID3v2::TextIdentificationFrame>(id, stringType);
...
void writeID3v2CommentsFrame(
        TagLib::ID3v2::Tag* pTag, const QString& text, const QString& description,
        bool isNumericOrURL = false) {
    TagLib::ID3v2::CommentsFrame* pFrame = findFirstCommentsFrame(*pTag, description, true);
    if (pFrame) {
        ...
        if (!text.isEmpty()) {
            const TagLib::String::Type stringType = getID3v2StringType(*pTag, isNumericOrURL);
            auto pFrame = std::make_unique<TagLib::ID3v2::CommentsFrame>(stringType);
            pFrame->setDescription(toTagLibString(description));
            pFrame->setText(toTagLibString(text));
            pTag->addFrame(pFrame.get());
            // Now that the plain pointer in pFrame is owned and managed by
    ...
    int numberOfRemovedCommentFrames = removeUserTextIdentificationFrames(pTag, "COMMENT");
...
void writeID3v2CommentsFrameWithoutDescription(
        TagLib::ID3v2::Tag* pTag, const QString& text, bool isNumericOrURL = false) {
    writeID3v2CommentsFrame(pTag, text, QString(), isNumericOrURL);
}

void writeID3v2UserTextIdentificationFrame(
        TagLib::ID3v2::Tag* pTag, const QString& description, const QString& text,
        bool isNumericOrURL = false) {
    ...
    if (pFrame) {
        ...
        if (!text.isEmpty()) {
            const TagLib::String::Type stringType = getID3v2StringType(*pTag, isNumericOrURL);
            auto pFrame = std::make_unique<TagLib::ID3v2::UserTextIdentificationFrame>(stringType);
            pFrame->setDescription(toTagLibString(description));
            pFrame->setText(toTagLibString(text));
            pTag->addFrame(pFrame.get());
            // Now that the plain pointer in pFrame is owned and managed by
...
void writeID3v2UniqueFileIdentifierFrame(
        TagLib::ID3v2::Tag* pTag, const QString& owner, const QByteArray& identifier) {
    ...
    if (pFrame) {
        ...
        // Add a new (non-empty) frame
        if (!identifier.isEmpty()) {
            auto pFrame = std::make_unique<TagLib::ID3v2::UniqueFileIdentifierFrame>(
...
bool readMP4Atom(
        const TagLib::MP4::Tag& tag, const TagLib::String& key, QString* pValue = nullptr) {
    const TagLib::MP4::ItemListMap::ConstIterator it(getItemListMap(tag).find(key));
...
void writeMP4Atom(TagLib::MP4::Tag* pTag, const TagLib::String& key, const TagLib::String& value) {
    if (value.isEmpty()) {
        // Purge empty atoms
...
void updateMP4Atom(TagLib::MP4::Tag* pTag, const TagLib::String& key, const TagLib::String& value) {
...
bool readAPEItem(
        const TagLib::APE::Tag& tag, const TagLib::String& key, QString* pValue = nullptr) {
    const TagLib::APE::ItemListMap::ConstIterator it(tag.itemListMap().find(key));
...
void writeAPEItem(TagLib::APE::Tag* pTag, const TagLib::String& key, const TagLib::String& value) {
    if (value.isEmpty()) {
        // Purge empty items
        pTag->removeItem(key);
    } else {
...
bool readXiphCommentField(
        const TagLib::Ogg::XiphComment& tag, const TagLib::String& key, QString* pValue) {
    const TagLib::Ogg::FieldListMap::ConstIterator it(tag.fieldListMap().find(key));
...
inline bool hasXiphCommentField(const TagLib::Ogg::XiphComment& tag, const TagLib::String& key) {
    return readXiphCommentField(tag, key, nullptr);
}

// Unconditionally write the field
void writeXiphCommentField(
        TagLib::Ogg::XiphComment* pTag, const TagLib::String& key, const TagLib::String& value) {
    if (value.isEmpty()) {
        // Purge empty fields
...
bool readAudioProperties(TrackMetadata* pTrackMetadata, const TagLib::File& file) {
    ...
    if (!pAudioProperties) {
        kLogger.warning() << "Failed to read audio properties from file" << file.name();
        return false;
    }
    ...
    for (const auto coverArtType : kPreferredVorbisCommentPictureTypes) {
        for (const auto pPicture : pictures) {
            ...
            if (pPicture->type() == coverArtType) {
                ...
                if (image.isNull()) {
                    kLogger.warning() << "Failed to load image from VorbisComment picture of type"
                                      << pPicture->type();
                    continue;
                } else {
...
const T* downcastID3v2Frame(TagLib::ID3v2::Frame* frame) {
    ...
    VERIFY_OR_DEBUG_ASSERT(downcastFrame) {
        ...
        kLogger.warning() << "Unexpected ID3v2" << frame->frameID().data() << "frame type";
    }
...
void importCoverImageFromID3v2Tag(QImage* pCoverArt, const TagLib::ID3v2::Tag& tag) {
    ...
    for (const auto coverArtType : kPreferredID3v2PictureTypes) {
        for (const auto pFrame : pFrames) {
            ...
            if (pApicFrame && (pApicFrame->type() == coverArtType)) {
                ...
                if (image.isNull()) {
                    kLogger.warning() << "Failed to load image from ID3v2 APIC frame of type"
                                      << pApicFrame->type();
                    continue;
                } else {
                    *pCoverArt = image;
    ...
    for (const auto pFrame : pFrames) {
        const auto* pApicFrame = downcastID3v2Frame<TagLib::ID3v2::AttachedPictureFrame>(pFrame);
        if (pApicFrame) {
...
void importCoverImageFromVorbisCommentTag(QImage* pCoverArt, TagLib::Ogg::XiphComment& tag) {
    ...
    if (tag.fieldListMap().contains("METADATA_BLOCK_PICTURE")) {
        ...
        const TagLib::StringList& base64EncodedList = tag.fieldListMap()["METADATA_BLOCK_PICTURE"];
#if (TAGLIB_HAS_VORBIS_COMMENT_PICTURES)
        if (!base64EncodedList.isEmpty()) {
            kLogger.warning() << "Taking legacy code path for reading cover art from VorbisComment "
    ...
    if (tag.fieldListMap().contains("COVERART")) {
        const TagLib::StringList& base64EncodedList = tag.fieldListMap()["COVERART"];
        if (!base64EncodedList.isEmpty()) {
            kLogger.warning() << "Fallback: Trying to parse image from deprecated VorbisComment "
                                 "field COVERART";
        }
        for (const auto& base64Encoded : base64EncodedList) {
...
void importCoverImageFromMP4Tag(QImage* pCoverArt, const TagLib::MP4::Tag& tag) {
    ...
    if (getItemListMap(tag).contains("covr")) {
        TagLib::MP4::CoverArtList coverArtList = getItemListMap(tag)["covr"].toCoverArtList();
...
void importTrackMetadataFromTag(
        TrackMetadata* pTrackMetadata, const TagLib::Tag& tag, int readMask) {
    if (!pTrackMetadata) {
        return; // nothing to do
    }
...
void importTrackMetadataFromID3v2Tag(TrackMetadata* pTrackMetadata, const TagLib::ID3v2::Tag& tag) {
    if (!pTrackMetadata) {
        return; // nothing to do
    }
    ...
    TagLib::ID3v2::CommentsFrame* pCommentsFrame = findFirstCommentsFrameWithoutDescription(tag);
    if (pCommentsFrame) {
        ...
        QString comment = readFirstUserTextIdentificationFrame(tag, "COMMENT");
        if (!comment.isNull()) {
    ...
    if (appleGroupingFrames.isEmpty()) {
        ...
        if (!traditionalGroupingFrames.isEmpty()) {
            pTrackMetadata->refTrackInfo().setGrouping(
                    toQStringFirstNotEmpty(traditionalGroupingFrames));
        }
    } else {
    ...
    const QString recordingTime(toQStringFirstNotEmpty(tag.frameListMap()["TDRC"]));
    if ((tag.header()->majorVersion() >= 4) && !recordingTime.isEmpty()) {
        pTrackMetadata->refTrackInfo().setYear(recordingTime);
    } else {
        // Fallback to TYER + TDAT
        const QString recordingYear(toQStringFirstNotEmpty(tag.frameListMap()["TYER"]).trimmed());
        ...
        if (ID3V2_TYER_FORMAT.length() == recordingYear.length()) {
            ...
            if (ID3V2_TDAT_FORMAT.length() == recordingDate.length()) {
                const QDate date(QDate::fromString(
                        recordingYear + recordingDate, ID3V2_TYER_FORMAT + ID3V2_TDAT_FORMAT));
                if (date.isValid()) {
    ...
    if (!trackNumberFrame.isEmpty()) {
        ...
        TrackNumbers::splitString(
                toQStringFirstNotEmpty(trackNumberFrame), &trackNumber, &trackTotal);
    ...
    if (!discNumberFrame.isEmpty()) {
        QString discNumber;
        QString discTotal;
        TrackNumbers::splitString(toQStringFirstNotEmpty(discNumberFrame), &discNumber, &discTotal);
    ...
    if (!bpmFrame.isEmpty()) {
        ...
        if (bpmValue != bpmValueOriginal) {
            kLogger.warning() << " Changing BPM on" << pTrackMetadata->getTrackInfo().getArtist()
                              << "-" << pTrackMetadata->getTrackInfo().getTitle() << "from"
                              << bpmValueOriginal << "to" << bpmValue;
        }
        pTrackMetadata->refTrackInfo().setBpm(Bpm(bpmValue));
    }
    ...
    QString trackGain = readFirstUserTextIdentificationFrame(tag, "REPLAYGAIN_TRACK_GAIN");
    if (!trackGain.isEmpty()) {
        parseTrackGain(pTrackMetadata, trackGain);
    }
    QString trackPeak = readFirstUserTextIdentificationFrame(tag, "REPLAYGAIN_TRACK_PEAK");
    if (!trackPeak.isEmpty()) {
        parseTrackPeak(pTrackMetadata, trackPeak);
    }

    QString albumGain = readFirstUserTextIdentificationFrame(tag, "REPLAYGAIN_ALBUM_GAIN");
    if (!albumGain.isEmpty()) {
        parseAlbumGain(pTrackMetadata, albumGain);
    }
    QString albumPeak = readFirstUserTextIdentificationFrame(tag, "REPLAYGAIN_ALBUM_PEAK");
    if (!albumPeak.isEmpty()) {
        parseAlbumPeak(pTrackMetadata, albumPeak);
    }

    QString trackArtistId = readFirstUserTextIdentificationFrame(tag, "MusicBrainz Artist Id");
    if (!trackArtistId.isNull()) {
        pTrackMetadata->refTrackInfo().setMusicBrainzArtistId(QUuid(trackArtistId));
    }
    QByteArray trackRecordingId = readFirstUniqueFileIdentifierFrame(tag, "http://musicbrainz.org");
    ...
    QString trackWorkId = readFirstUserTextIdentificationFrame(tag, "MusicBrainz Work Id");
    ...
    QString albumReleaseId = readFirstUserTextIdentificationFrame(tag, "MusicBrainz Album Id");
    ...
    if (!encoderSettingsFrame.isEmpty()) {
        pTrackMetadata->refTrackInfo().setEncoderSettings(
                toQStringFirstNotEmpty(encoderSettingsFrame));
    }
}
...
void importTrackMetadataFromAPETag(TrackMetadata* pTrackMetadata, const TagLib::APE::Tag& tag) {
    ...
    QString composer;
    if (readAPEItem(tag, "Composer", &composer) || readAPEItem(tag, "COMPOSER", &composer)) {
        pTrackMetadata->refTrackInfo().setComposer(composer);
    }

    QString grouping;
    if (readAPEItem(tag, "Grouping", &grouping) || readAPEItem(tag, "GROUPING", &grouping)) {
    ...
    // https://picard.musicbrainz.org/docs/mappings
    QString year;
    if (readAPEItem(tag, "Year", &year) || readAPEItem(tag, "YEAR", &year)) {
        pTrackMetadata->refTrackInfo().setYear(year);
    }

    QString trackNumber;
    if (readAPEItem(tag, "Track", &trackNumber) || readAPEItem(tag, "TRACK", &trackNumber)) {
        QString trackTotal;
    ...
    QString discNumber;
    if (readAPEItem(tag, "Disc", &discNumber) || readAPEItem(tag, "DISC", &discNumber)) {
        QString discTotal;
        TrackNumbers::splitString(discNumber, &discNumber, &discTotal);
        pTrackMetadata->refTrackInfo().setDiscNumber(discNumber);
        pTrackMetadata->refTrackInfo().setDiscTotal(discTotal);
    }
    ...
    QString conductor;
    if (readAPEItem(tag, "Conductor", &conductor) || readAPEItem(tag, "CONDUCTOR", &conductor)) {
    ...
    QString language;
    if (readAPEItem(tag, "Language", &language) || readAPEItem(tag, "LANGUAGE", &language)) {
        pTrackMetadata->refTrackInfo().setLanguage(language);
    }
    QString lyricist;
    if (readAPEItem(tag, "Lyricist", &lyricist) || readAPEItem(tag, "LYRICIST", &lyricist)) {
        pTrackMetadata->refTrackInfo().setLyricist(lyricist);
    }
    QString mood;
    if (readAPEItem(tag, "Mood", &mood) || readAPEItem(tag, "MOOD", &mood)) {
        pTrackMetadata->refTrackInfo().setMood(mood);
    }
    QString remixer;
    if (readAPEItem(tag, "MixArtist", &remixer) || readAPEItem(tag, "MIXARTIST", &remixer) ||
        readAPEItem(tag, "REMIXER", &remixer)) {
        pTrackMetadata->refTrackInfo().setRemixer(remixer);
    }
    QString copyright;
    if (readAPEItem(tag, "Copyright", &copyright) || readAPEItem(tag, "COPYRIGHT", &copyright)) {
        pTrackMetadata->refAlbumInfo().setCopyright(copyright);
    }
    QString license;
    if (readAPEItem(tag, "License", &license) || readAPEItem(tag, "LICENSE", &license)) {
        pTrackMetadata->refAlbumInfo().setLicense(license);
    }
    QString recordLabel;
    if (readAPEItem(tag, "Label", &recordLabel) || readAPEItem(tag, "LABEL", &recordLabel)) {
        pTrackMetadata->refAlbumInfo().setRecordLabel(recordLabel);
    }
    QString subtitle;
    if (readAPEItem(tag, "Subtitle", &subtitle) || readAPEItem(tag, "SUBTITLE", &subtitle)) {
        pTrackMetadata->refTrackInfo().setSubtitle(subtitle);
    }
    QString encoder;
    if (readAPEItem(tag, "EncodedBy", &encoder) || readAPEItem(tag, "ENCODEDBY", &encoder)) {
...
void importTrackMetadataFromVorbisCommentTag(
        TrackMetadata* pTrackMetadata, const TagLib::Ogg::XiphComment& tag) {
    if (!pTrackMetadata) {
        return; // nothing to do
    }
    ...
    if (readXiphCommentField(tag, "ALBUMARTIST", &albumArtist) || // recommended field
        readXiphCommentField(
                tag, "ALBUM_ARTIST",
                &albumArtist) || // alternative field (with underscore character)
        readXiphCommentField(
                tag, "ALBUM ARTIST", &albumArtist) || // alternative field (with space character)
    ...
    // Mixxx (<= 2.0) favored "TEMPO":
    // https://www.mixxx.org/wiki/doku.php/library_metadata_rewrite_using_taglib
    QString bpm;
...
void importTrackMetadataFromMP4Tag(TrackMetadata* pTrackMetadata, const TagLib::MP4::Tag& tag) {
    ...
    QString key;
    if (readMP4Atom(
                tag, "----:com.apple.iTunes:initialkey",
                &key) || // preferred (conforms to MixedInKey, Serato, Traktor)
        readMP4Atom(
                tag, "----:com.apple.iTunes:KEY",
    ...
    QString albumReleaseGroupId;
    if (readMP4Atom(
                tag, "----:com.apple.iTunes:MusicBrainz Release Group Id", &albumReleaseGroupId)) {
        pTrackMetadata->refAlbumInfo().setMusicBrainzReleaseGroupId(QUuid(albumReleaseGroupId));
    }
...
void importTrackMetadataFromRIFFTag(
        TrackMetadata* pTrackMetadata, const TagLib::RIFF::Info::Tag& tag) {
    // Just delegate to the common import function
    importTrackMetadataFromTag(pTrackMetadata, tag);
}

void exportTrackMetadataIntoTag(
        TagLib::Tag* pTag, const TrackMetadata& trackMetadata, int writeMask) {
    ...
    if (0 == (writeMask & WRITE_TAG_OMIT_TRACK_NUMBER)) {
        ...
        const TrackNumbers::ParseResult parseResult = TrackNumbers::parseFromString(
                trackMetadata.getTrackInfo().getTrackNumber(), &parsedTrackNumbers);
...
bool exportTrackMetadataIntoID3v2Tag(TagLib::ID3v2::Tag* pTag, const TrackMetadata& trackMetadata) {
    if (!pTag) {
        return false;
    }
    ...
    if ((pHeader->majorVersion() >= 4) || !pTag->frameList("TDRC").isEmpty()) {
        writeID3v2TextIdentificationFrame(pTag, "TDRC", trackMetadata.getTrackInfo().getYear());
    }
    if (pHeader->majorVersion() < 4) {
        ...
        if (date.isValid()) {
            ...
            const QString calendarYear(TrackMetadata::formatCalendarYear(
                    trackMetadata.getTrackInfo().getYear(), &calendarYearValid));
    ...
    writeID3v2TextIdentificationFrame(pTag, "TPE2", trackMetadata.getAlbumInfo().getArtist());
    writeID3v2TextIdentificationFrame(pTag, "TCOM", trackMetadata.getTrackInfo().getComposer());
    ...
    if (trackMetadata.getTrackInfo().getWork().isNull() && pTag->frameListMap()["GRP1"].isEmpty()) {
        // Traditional mapping
        writeID3v2TextIdentificationFrame(pTag, "TIT1", trackMetadata.getTrackInfo().getGrouping());
    } else {
        // New mapping
        writeID3v2TextIdentificationFrame(pTag, "GRP1", trackMetadata.getTrackInfo().getGrouping());
        writeID3v2TextIdentificationFrameStringIfNotNull(
                pTag, "TIT1", trackMetadata.getTrackInfo().getWork());
    }
    writeID3v2TextIdentificationFrameStringIfNotNull(
            pTag, "MVNM", trackMetadata.getTrackInfo().getMovement());
    ...
    writeID3v2TextIdentificationFrame(pTag, "TBPM", formatBpmInteger(trackMetadata), true);

    writeID3v2TextIdentificationFrame(pTag, "TKEY", trackMetadata.getTrackInfo().getKey());

    writeID3v2UserTextIdentificationFrame(
            pTag, "REPLAYGAIN_TRACK_GAIN", formatTrackGain(trackMetadata), true);
    ...
    if (hasTrackPeak(trackMetadata)) {
        writeID3v2UserTextIdentificationFrame(
                pTag, "REPLAYGAIN_TRACK_PEAK", formatTrackPeak(trackMetadata), true);
    }
    ...
    if (hasAlbumGain(trackMetadata)) {
        writeID3v2UserTextIdentificationFrame(
                pTag, "REPLAYGAIN_ALBUM_GAIN", formatAlbumGain(trackMetadata), true);
    }
    if (hasAlbumPeak(trackMetadata)) {
        writeID3v2UserTextIdentificationFrame(
                pTag, "REPLAYGAIN_ALBUM_PEAK", formatAlbumPeak(trackMetadata), true);
    }

    if (!trackMetadata.getTrackInfo().getMusicBrainzArtistId().isNull()) {
        writeID3v2UserTextIdentificationFrame(
                pTag, "MusicBrainz Artist Id",
                trackMetadata.getTrackInfo().getMusicBrainzArtistId().toString(), false);
    }
    if (!trackMetadata.getTrackInfo().getMusicBrainzRecordingId().isNull()) {
        QByteArray identifier =
                trackMetadata.getTrackInfo().getMusicBrainzRecordingId().toByteArray();
        if (identifier.size() == 38) {
            // Strip leading/trailing curly braces
            identifier = identifier.mid(1, 36);
        }
        DEBUG_ASSERT(identifier.size() == 36);
        writeID3v2UniqueFileIdentifierFrame(pTag, "http://musicbrainz.org", identifier);
    }
    if (!trackMetadata.getTrackInfo().getMusicBrainzReleaseId().isNull()) {
        writeID3v2UserTextIdentificationFrame(
                pTag, "MusicBrainz Release Track Id",
                trackMetadata.getTrackInfo().getMusicBrainzReleaseId().toString(), false);
    }
    if (!trackMetadata.getTrackInfo().getMusicBrainzWorkId().isNull()) {
        writeID3v2UserTextIdentificationFrame(
                pTag, "MusicBrainz Work Id",
                trackMetadata.getTrackInfo().getMusicBrainzWorkId().toString(), false);
    }
    if (!trackMetadata.getAlbumInfo().getMusicBrainzArtistId().isNull()) {
        writeID3v2UserTextIdentificationFrame(
                pTag, "MusicBrainz Album Artist Id",
                trackMetadata.getAlbumInfo().getMusicBrainzArtistId().toString(), false);
    }
    if (!trackMetadata.getAlbumInfo().getMusicBrainzReleaseId().isNull()) {
        writeID3v2UserTextIdentificationFrame(
                pTag, "MusicBrainz Album Id",
                trackMetadata.getAlbumInfo().getMusicBrainzReleaseId().toString(), false);
    }
    if (!trackMetadata.getAlbumInfo().getMusicBrainzReleaseGroupId().isNull()) {
        writeID3v2UserTextIdentificationFrame(
                pTag, "MusicBrainz Release Group Id",
                trackMetadata.getAlbumInfo().getMusicBrainzReleaseGroupId().toString(), false);
    }
    ...
    writeID3v2TextIdentificationFrameStringIfNotNull(
            pTag, "TPE3", trackMetadata.getTrackInfo().getConductor());
    writeID3v2TextIdentificationFrameStringIfNotNull(
            pTag, "TSRC", trackMetadata.getTrackInfo().getISRC());
    writeID3v2TextIdentificationFrameStringIfNotNull(
            pTag, "TLAN", trackMetadata.getTrackInfo().getLanguage());
    writeID3v2TextIdentificationFrameStringIfNotNull(
            pTag, "TEXT", trackMetadata.getTrackInfo().getLyricist());
    if (pHeader->majorVersion() >= 4) {
        writeID3v2TextIdentificationFrameStringIfNotNull(
                pTag, "TMOO", trackMetadata.getTrackInfo().getMood());
    }
    writeID3v2TextIdentificationFrameStringIfNotNull(
            pTag, "TCOP", trackMetadata.getAlbumInfo().getCopyright());
    writeID3v2TextIdentificationFrameStringIfNotNull(
            pTag, "WCOP", trackMetadata.getAlbumInfo().getLicense());
    writeID3v2TextIdentificationFrameStringIfNotNull(
            pTag, "TPUB", trackMetadata.getAlbumInfo().getRecordLabel());
    writeID3v2TextIdentificationFrameStringIfNotNull(
            pTag, "TPE4", trackMetadata.getTrackInfo().getRemixer());
    writeID3v2TextIdentificationFrameStringIfNotNull(
            pTag, "TIT3", trackMetadata.getTrackInfo().getSubtitle());
    writeID3v2TextIdentificationFrameStringIfNotNull(
            pTag, "TENC", trackMetadata.getTrackInfo().getEncoder());
    writeID3v2TextIdentificationFrameStringIfNotNull(
...
bool exportTrackMetadataIntoAPETag(TagLib::APE::Tag* pTag, const TrackMetadata& trackMetadata) {
    ...
    exportTrackMetadataIntoTag(
            pTag, trackMetadata, WRITE_TAG_OMIT_TRACK_NUMBER | WRITE_TAG_OMIT_YEAR);
    ...
    writeAPEItem(pTag, "Year", toTagLibString(trackMetadata.getTrackInfo().getYear()));

    writeAPEItem(pTag, "Album Artist", toTagLibString(trackMetadata.getAlbumInfo().getArtist()));
    writeAPEItem(pTag, "Composer", toTagLibString(trackMetadata.getTrackInfo().getComposer()));
    writeAPEItem(pTag, "Grouping", toTagLibString(trackMetadata.getTrackInfo().getGrouping()));

    writeAPEItem(pTag, "BPM", toTagLibString(formatBpm(trackMetadata)));

    writeAPEItem(pTag, "INITIALKEY", toTagLibString(trackMetadata.getTrackInfo().getKey()));

    writeAPEItem(pTag, "REPLAYGAIN_TRACK_GAIN", toTagLibString(formatTrackGain(trackMetadata)));
    // NOTE(uklotzde, 2018-04-22): The analyzers currently doesn't
    // calculate a peak value, so leave it untouched in the file if
    // the value is invalid/absent. Otherwise the APE item would be
    // deleted.
    if (hasTrackPeak(trackMetadata)) {
        writeAPEItem(pTag, "REPLAYGAIN_TRACK_PEAK", toTagLibString(formatTrackPeak(trackMetadata)));
    }
    ...
    if (hasAlbumGain(trackMetadata)) {
        writeAPEItem(pTag, "REPLAYGAIN_ALBUM_GAIN", toTagLibString(formatAlbumGain(trackMetadata)));
    }
    if (hasAlbumPeak(trackMetadata)) {
        writeAPEItem(pTag, "REPLAYGAIN_ALBUM_PEAK", toTagLibString(formatAlbumPeak(trackMetadata)));
    }

    if (!trackMetadata.getTrackInfo().getMusicBrainzArtistId().isNull()) {
        writeAPEItem(
                pTag, "MUSICBRAINZ_ARTISTID",
                toTagLibString(trackMetadata.getTrackInfo().getMusicBrainzArtistId().toString()));
    }
    if (!trackMetadata.getTrackInfo().getMusicBrainzRecordingId().isNull()) {
        writeAPEItem(
                pTag, "MUSICBRAINZ_TRACKID",
                toTagLibString(
                        trackMetadata.getTrackInfo().getMusicBrainzRecordingId().toString()));
    }
    if (!trackMetadata.getTrackInfo().getMusicBrainzReleaseId().isNull()) {
        writeAPEItem(
                pTag, "MUSICBRAINZ_RELEASETRACKID",
                toTagLibString(trackMetadata.getTrackInfo().getMusicBrainzReleaseId().toString()));
    }
    if (!trackMetadata.getTrackInfo().getMusicBrainzWorkId().isNull()) {
        writeAPEItem(
                pTag, "MUSICBRAINZ_WORKID",
                toTagLibString(trackMetadata.getTrackInfo().getMusicBrainzWorkId().toString()));
    }
    if (!trackMetadata.getAlbumInfo().getMusicBrainzArtistId().isNull()) {
        writeAPEItem(
                pTag, "MUSICBRAINZ_ALBUMARTISTID",
                toTagLibString(trackMetadata.getAlbumInfo().getMusicBrainzArtistId().toString()));
    }
    if (!trackMetadata.getAlbumInfo().getMusicBrainzReleaseId().isNull()) {
        writeAPEItem(
                pTag, "MUSICBRAINZ_ALBUMID",
                toTagLibString(trackMetadata.getAlbumInfo().getMusicBrainzReleaseId().toString()));
    }
    if (!trackMetadata.getAlbumInfo().getMusicBrainzReleaseGroupId().isNull()) {
        writeAPEItem(
                pTag, "MUSICBRAINZ_RELEASEGROUPID",
                toTagLibString(
    ...
    if (!trackMetadata.getTrackInfo().getConductor().isNull()) {
        writeAPEItem(
                pTag, "Conductor", toTagLibString(trackMetadata.getTrackInfo().getConductor()));
    }
    if (!trackMetadata.getTrackInfo().getISRC().isNull()) {
        writeAPEItem(pTag, "ISRC", toTagLibString(trackMetadata.getTrackInfo().getISRC()));
    }
    if (!trackMetadata.getTrackInfo().getLanguage().isNull()) {
        writeAPEItem(pTag, "Language", toTagLibString(trackMetadata.getTrackInfo().getLanguage()));
    }
    if (!trackMetadata.getTrackInfo().getLyricist().isNull()) {
        writeAPEItem(pTag, "Lyricist", toTagLibString(trackMetadata.getTrackInfo().getLyricist()));
    }
    if (!trackMetadata.getTrackInfo().getMood().isNull()) {
        writeAPEItem(pTag, "Mood", toTagLibString(trackMetadata.getTrackInfo().getMood()));
    }
    if (!trackMetadata.getAlbumInfo().getCopyright().isNull()) {
        writeAPEItem(
                pTag, "Copyright", toTagLibString(trackMetadata.getAlbumInfo().getCopyright()));
    }
    if (!trackMetadata.getAlbumInfo().getLicense().isNull()) {
        writeAPEItem(pTag, "LICENSE", toTagLibString(trackMetadata.getAlbumInfo().getLicense()));
    }
    if (!trackMetadata.getAlbumInfo().getRecordLabel().isNull()) {
        writeAPEItem(pTag, "Label", toTagLibString(trackMetadata.getAlbumInfo().getRecordLabel()));
    }
    if (!trackMetadata.getTrackInfo().getRemixer().isNull()) {
        writeAPEItem(pTag, "MixArtist", toTagLibString(trackMetadata.getTrackInfo().getRemixer()));
    }
    if (!trackMetadata.getTrackInfo().getSubtitle().isNull()) {
        writeAPEItem(pTag, "Subtitle", toTagLibString(trackMetadata.getTrackInfo().getSubtitle()));
    }
    if (!trackMetadata.getTrackInfo().getEncoder().isNull()) {
        writeAPEItem(pTag, "EncodedBy", toTagLibString(trackMetadata.getTrackInfo().getEncoder()));
    }
    if (!trackMetadata.getTrackInfo().getEncoderSettings().isNull()) {
        writeAPEItem(
                pTag, "EncoderSettings",
...
bool exportTrackMetadataIntoXiphComment(
        TagLib::Ogg::XiphComment* pTag, const TrackMetadata& trackMetadata) {
    if (!pTag) {
        return false;
    }
    ...
    if (hasXiphCommentField(*pTag, "COMMENT") || !hasXiphCommentField(*pTag, "DESCRIPTION")) {
        ...
        writeXiphCommentField(
                pTag, "COMMENT", toTagLibString(trackMetadata.getTrackInfo().getComment()));
    } else {
        // Preserve and update the "DESCRIPTION" field only if it already exists
        DEBUG_ASSERT(hasXiphCommentField(*pTag, "DESCRIPTION"));
        writeXiphCommentField(
                pTag, "DESCRIPTION", toTagLibString(trackMetadata.getTrackInfo().getComment()));
    }

    // Write unambiguous fields
    writeXiphCommentField(pTag, "DATE", toTagLibString(trackMetadata.getTrackInfo().getYear()));
    writeXiphCommentField(
            pTag, "COMPOSER", toTagLibString(trackMetadata.getTrackInfo().getComposer()));
    writeXiphCommentField(
            pTag, "GROUPING", toTagLibString(trackMetadata.getTrackInfo().getGrouping()));
    writeXiphCommentField(
            pTag, "TRACKNUMBER", toTagLibString(trackMetadata.getTrackInfo().getTrackNumber()));

    // According to https://wiki.xiph.org/Field_names "TRACKTOTAL" is
    // the proposed field name, but some applications use "TOTALTRACKS".
    const TagLib::String trackTotal(toTagLibString(trackMetadata.getTrackInfo().getTrackTotal()));
    ...
    const TagLib::String albumArtist(toTagLibString(trackMetadata.getAlbumInfo().getArtist()));
    ...
    const TagLib::String bpm(toTagLibString(formatBpm(trackMetadata)));
    // MusicBrainz recommends "BPM": https://picard.musicbrainz.org/docs/mappings
    // Mixxx (<= 2.0) favored "TEMPO":
    // https://www.mixxx.org/wiki/doku.php/library_metadata_rewrite_using_taglib
    ...
    writeXiphCommentField(
            pTag, "REPLAYGAIN_TRACK_GAIN", toTagLibString(formatTrackGain(trackMetadata)));
    // NOTE(uklotzde, 2018-04-22): The analyzers currently doesn't
    // calculate a peak value, so leave it untouched in the file if
    ...
    if (hasTrackPeak(trackMetadata)) {
        writeXiphCommentField(
                pTag, "REPLAYGAIN_TRACK_PEAK", toTagLibString(formatTrackPeak(trackMetadata)));
    }
    ...
    if (hasAlbumGain(trackMetadata)) {
        writeXiphCommentField(
                pTag, "REPLAYGAIN_ALBUM_GAIN", toTagLibString(formatAlbumGain(trackMetadata)));
    }
    if (hasAlbumPeak(trackMetadata)) {
        writeXiphCommentField(
                pTag, "REPLAYGAIN_ALBUM_PEAK", toTagLibString(formatAlbumPeak(trackMetadata)));
    }
    if (!trackMetadata.getTrackInfo().getMusicBrainzArtistId().isNull()) {
        writeXiphCommentField(
                pTag, "MUSICBRAINZ_ARTISTID",
    ...
    if (!trackMetadata.getTrackInfo().getISRC().isNull()) {
        writeXiphCommentField(pTag, "ISRC", toTagLibString(trackMetadata.getTrackInfo().getISRC()));
    }
    if (!trackMetadata.getTrackInfo().getLanguage().isNull()) {
        writeXiphCommentField(
                pTag, "LANGUAGE", toTagLibString(trackMetadata.getTrackInfo().getLanguage()));
    }
    if (!trackMetadata.getTrackInfo().getLyricist().isNull()) {
        writeXiphCommentField(
                pTag, "LYRICIST", toTagLibString(trackMetadata.getTrackInfo().getLyricist()));
    }
    if (!trackMetadata.getTrackInfo().getMood().isNull()) {
        writeXiphCommentField(pTag, "MOOD", toTagLibString(trackMetadata.getTrackInfo().getMood()));
    }
    if (!trackMetadata.getAlbumInfo().getCopyright().isNull()) {
        writeXiphCommentField(
                pTag, "COPYRIGHT", toTagLibString(trackMetadata.getAlbumInfo().getCopyright()));
    ...
    if (!trackMetadata.getAlbumInfo().getLicense().isNull()) {
        writeXiphCommentField(
                pTag, "LICENSE", toTagLibString(trackMetadata.getAlbumInfo().getLicense()));
    }
    if (!trackMetadata.getAlbumInfo().getRecordLabel().isNull()) {
        writeXiphCommentField(
                pTag, "LABEL", toTagLibString(trackMetadata.getAlbumInfo().getRecordLabel()));
    }
    if (!trackMetadata.getTrackInfo().getRemixer().isNull()) {
        writeXiphCommentField(
                pTag, "REMIXER", toTagLibString(trackMetadata.getTrackInfo().getRemixer()));
    }
    if (!trackMetadata.getTrackInfo().getSubtitle().isNull()) {
        writeXiphCommentField(
                pTag, "SUBTITLE", toTagLibString(trackMetadata.getTrackInfo().getSubtitle()));
    }
    if (!trackMetadata.getTrackInfo().getEncoder().isNull()) {
        writeXiphCommentField(
                pTag, "ENCODEDBY", toTagLibString(trackMetadata.getTrackInfo().getEncoder()));
    }
    if (!trackMetadata.getTrackInfo().getEncoderSettings().isNull()) {
        writeXiphCommentField(
                pTag, "ENCODERSETTINGS",
...
bool exportTrackMetadataIntoMP4Tag(TagLib::MP4::Tag* pTag, const TrackMetadata& trackMetadata) {
    ...
    TrackNumbers parsedTrackNumbers;
    const TrackNumbers::ParseResult trackParseResult = TrackNumbers::parseFromStrings(
            trackMetadata.getTrackInfo().getTrackNumber(),
            trackMetadata.getTrackInfo().getTrackTotal(), &parsedTrackNumbers);
    switch (trackParseResult) {
        ...
        case TrackNumbers::ParseResult::VALID:
            pTag->itemListMap()["trkn"] = TagLib::MP4::Item(
                    parsedTrackNumbers.getActual(), parsedTrackNumbers.getTotal());
            break;
        default:
    ...
    if (trackMetadata.getTrackInfo().getBpm().hasValue()) {
        ...
        const int tmpoValue = Bpm::valueToInteger(trackMetadata.getTrackInfo().getBpm().getValue());
        pTag->itemListMap()["tmpo"] = tmpoValue;
    } else {
        pTag->itemListMap().erase("tmpo");
    }
    writeMP4Atom(pTag, "----:com.apple.iTunes:BPM", toTagLibString(formatBpm(trackMetadata)));
    ...
    if (hasTrackPeak(trackMetadata)) {
        writeMP4Atom(
                pTag, "----:com.apple.iTunes:replaygain_track_peak",
    ...
    if (hasAlbumGain(trackMetadata)) {
        writeMP4Atom(
                pTag, "----:com.apple.iTunes:replaygain_album_gain",
                toTagLibString(formatAlbumGain(trackMetadata)));
    }
    if (hasAlbumPeak(trackMetadata)) {
        writeMP4Atom(
                pTag, "----:com.apple.iTunes:replaygain_album_peak",
                toTagLibString(formatAlbumPeak(trackMetadata)));
    }
    if (!trackMetadata.getTrackInfo().getMusicBrainzArtistId().isNull()) {
        writeMP4Atom(
                pTag, "----:com.apple.iTunes:MusicBrainz Artist Id",
                toTagLibString(trackMetadata.getTrackInfo().getMusicBrainzArtistId().toString()));
    }
    if (!trackMetadata.getTrackInfo().getMusicBrainzRecordingId().isNull()) {
        writeMP4Atom(
                pTag, "----:com.apple.iTunes:MusicBrainz Track Id",
                toTagLibString(
                        trackMetadata.getTrackInfo().getMusicBrainzRecordingId().toString()));
    }
    if (!trackMetadata.getTrackInfo().getMusicBrainzReleaseId().isNull()) {
        writeMP4Atom(
                pTag, "----:com.apple.iTunes:MusicBrainz Release Track Id",
                toTagLibString(trackMetadata.getTrackInfo().getMusicBrainzReleaseId().toString()));
    }
    if (!trackMetadata.getTrackInfo().getMusicBrainzWorkId().isNull()) {
        writeMP4Atom(
                pTag, "----:com.apple.iTunes:MusicBrainz Work Id",
                toTagLibString(trackMetadata.getTrackInfo().getMusicBrainzWorkId().toString()));
    }
    if (!trackMetadata.getAlbumInfo().getMusicBrainzArtistId().isNull()) {
        writeMP4Atom(
                pTag, "----:com.apple.iTunes:MusicBrainz Album Artist Id",
                toTagLibString(trackMetadata.getAlbumInfo().getMusicBrainzArtistId().toString()));
    }
    if (!trackMetadata.getAlbumInfo().getMusicBrainzReleaseId().isNull()) {
        writeMP4Atom(
                pTag, "----:com.apple.iTunes:MusicBrainz Album Id",
                toTagLibString(trackMetadata.getAlbumInfo().getMusicBrainzReleaseId().toString()));
    }
    if (!trackMetadata.getAlbumInfo().getMusicBrainzReleaseGroupId().isNull()) {
        writeMP4Atom(
                pTag, "----:com.apple.iTunes:MusicBrainz Release Group Id",
                toTagLibString(
                        trackMetadata.getAlbumInfo().getMusicBrainzReleaseGroupId().toString()));
    }
    if (!trackMetadata.getTrackInfo().getConductor().isNull()) {
        writeMP4Atom(
                pTag, "----:com.apple.iTunes:CONDUCTOR",
                toTagLibString(trackMetadata.getTrackInfo().getConductor()));
    }
    if (!trackMetadata.getTrackInfo().getISRC().isNull()) {
        writeMP4Atom(
                pTag, "----:com.apple.iTunes:ISRC",
                toTagLibString(trackMetadata.getTrackInfo().getISRC()));
    }
    if (!trackMetadata.getTrackInfo().getLanguage().isNull()) {
        writeMP4Atom(
                pTag, "----:com.apple.iTunes:LANGUAGE",
                toTagLibString(trackMetadata.getTrackInfo().getLanguage()));
    }
    if (!trackMetadata.getTrackInfo().getLyricist().isNull()) {
        writeMP4Atom(
                pTag, "----:com.apple.iTunes:LYRICIST",
                toTagLibString(trackMetadata.getTrackInfo().getLyricist()));
    }
    if (!trackMetadata.getTrackInfo().getMood().isNull()) {
        writeMP4Atom(
                pTag, "----:com.apple.iTunes:MOOD",
                toTagLibString(trackMetadata.getTrackInfo().getMood()));
    }
    if (!trackMetadata.getAlbumInfo().getCopyright().isNull()) {
        writeMP4Atom(pTag, "cprt", toTagLibString(trackMetadata.getAlbumInfo().getCopyright()));
    }
    if (!trackMetadata.getAlbumInfo().getLicense().isNull()) {
        writeMP4Atom(
                pTag, "----:com.apple.iTunes:LICENSE",
                toTagLibString(trackMetadata.getAlbumInfo().getLicense()));
    }
    if (!trackMetadata.getAlbumInfo().getRecordLabel().isNull()) {
        writeMP4Atom(
                pTag, "----:com.apple.iTunes:LABEL",
                toTagLibString(trackMetadata.getAlbumInfo().getRecordLabel()));
    }
    if (!trackMetadata.getTrackInfo().getRemixer().isNull()) {
        writeMP4Atom(
                pTag, "----:com.apple.iTunes:REMIXER",
                toTagLibString(trackMetadata.getTrackInfo().getRemixer()));
    }
    if (!trackMetadata.getTrackInfo().getSubtitle().isNull()) {
        writeMP4Atom(
                pTag, "----:com.apple.iTunes:SUBTITLE",
                toTagLibString(trackMetadata.getTrackInfo().getSubtitle()));
    }
    if (!trackMetadata.getTrackInfo().getEncoder().isNull()) {
        writeMP4Atom(pTag, "\251too", toTagLibString(trackMetadata.getTrackInfo().getEncoder()));
    }

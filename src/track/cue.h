...
class Cue : public QObject {
  ...
  public:
    ...
    enum CueType {
        ...
        INTRO   = 6,
        OUTRO   = 7,
        // The loop that's automatically saved when unloading a track.
        LOADLOOP = 8,
    ...
    // Safely setCueType from it's int representation.
    // If the int does not correspond to any valid type, the type is set
    // to INVALID.
  ...
  private:
    ...
    static const int iCueTypeMaxInt = 8;
